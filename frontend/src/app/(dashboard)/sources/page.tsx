...
export default function SourcesPage() {
  ...
  const fetchSources = useCallback(async (reset = false) => {
    try {
      ...
      setError(t('sources.load_failed'))
      toast.error(t('sources.load_failed'))
    } finally {
      setLoading(false)
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      toast.error(t('sources.delete.failed'))
    }
  }
    ...
    <AppShell>
      <div className="flex flex-col h-full w-full max-w-none px-6 py-6">
        <div className="mb-6 flex-shrink-0">
          <h1 className="text-3xl font-bold">{t('sources.all_title')}</h1>
            ...
            <thead className="sticky top-0 bg-background z-10">
              <tr className="border-b bg-muted/50">
                ...
                <th className="h-12 px-4 text-center align-middle font-medium text-muted-foreground hidden md:table-cell">
                  {t('sources.table.insights')}
                </th>
                <th className="h-12 px-4 text-center align-middle font-medium text-muted-foreground hidden lg:table-cell">
                  {t('sources.table.embedded')}
                </th>
                ...
                <tr>
                  <td colSpan={6} className="h-16 text-center">
                    <div className="flex items-center justify-center">
                      <LoadingSpinner />
                      <span className="ml-2 text-muted-foreground">{t('sources.loading_more')}</span>
                    </div>
                  </td>
                </tr>
              )}
      ...
      <ConfirmDialog
        ...
        description={t('sources.delete.desc', {
          name: deleteDialog.source?.title || t('sources.delete.this_source')
        })}
