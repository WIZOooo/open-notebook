      ...
      <CardContent className="space-y-6">
        ...
        {!isRebuildActive && (
          <div className="space-y-6">
            <div className="space-y-3">
              <Label htmlFor="mode">{t('advanced.rebuild.mode.label')}</Label>
              <Select value={mode} onValueChange={(value) => setMode(value as 'existing' | 'all')}>
                <SelectTrigger id="mode">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="existing">{t('advanced.rebuild.mode.existing')}</SelectItem>
                  <SelectItem value="all">{t('advanced.rebuild.mode.all')}</SelectItem>
                </SelectContent>
              </Select>
              <p className="text-sm text-muted-foreground">
                {mode === 'existing'
                  ? t('advanced.rebuild.mode.help_existing')
                  : t('advanced.rebuild.mode.help_all')}
              </p>
            </div>

            <div className="space-y-3">
              <Label>{t('advanced.rebuild.include.label')}</Label>
              <div className="space-y-3">
              ...
              {!isAnyTypeSelected && (
                <Alert variant="destructive">
                  ...
                  <AlertDescription>
                    {t('advanced.rebuild.validation.select_one')}
                  </AlertDescription>
                </Alert>
              )}
            </div>
            ...
            {rebuildMutation.isError && (
              <Alert variant="destructive">
                <AlertCircle className="h-4 w-4" />
                <AlertDescription>
                  {t('advanced.rebuild.start_failed', {
                    error: (rebuildMutation.error as Error)?.message || t('common.unknown_error'),
                  })}
                </AlertDescription>
              </Alert>
            )}
          ...
          <div className="space-y-4">
                ...
                <div className="flex flex-col">
                  ...
                  {status.status === 'running' && (
                    <span className="text-sm text-muted-foreground">
                      {t('advanced.rebuild.status.running_hint')}
                    </span>
                  )}
                </div>
              </div>
            ...
            {progressData && (
              <div className="space-y-2">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium">
                    {t('advanced.rebuild.progress_value', {
                ...
                {failedItems > 0 && (
                  <p className="text-sm text-yellow-600">
                    {t('advanced.rebuild.progress_failed', { count: failedItems })}
                  </p>
                )}
              </div>
            )}
            ...
            {stats && (
              <div className="grid grid-cols-4 gap-4">
                <div className="space-y-1">
                  <p className="text-sm text-muted-foreground">{t('common.sources')}</p>
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-2xl font-bold">
                    {processingTimeSeconds !== undefined
                      ? t('advanced.rebuild.seconds', { seconds: processingTimeSeconds.toFixed(1) })
                      : '—'}
                  </p>
                </div>
              </div>
            ...
            {status.started_at && (
              <div className="text-sm text-muted-foreground space-y-1">
                <p>{t('advanced.rebuild.started_at', { time: new Date(status.started_at).toLocaleString() })}</p>
                {status.completed_at && (
                  <p>{t('advanced.rebuild.completed_at', { time: new Date(status.completed_at).toLocaleString() })}</p>
                )}
              </div>
            )}
          </div>
        ...
        <Accordion type="single" collapsible className="w-full">
          <AccordionItem value="when">
            <AccordionTrigger>{t('advanced.rebuild.faq.when.title')}</AccordionTrigger>
            <AccordionContent className="space-y-2 text-sm">
              <p><strong>{t('advanced.rebuild.faq.when.lead')}</strong></p>
              <ul className="list-disc list-inside space-y-1 ml-2">
                <li><strong>{t('advanced.rebuild.faq.when.li1_title')}</strong> {t('advanced.rebuild.faq.when.li1_desc')}</li>
                <li><strong>{t('advanced.rebuild.faq.when.li2_title')}</strong> {t('advanced.rebuild.faq.when.li2_desc')}</li>
                <li><strong>{t('advanced.rebuild.faq.when.li3_title')}</strong> {t('advanced.rebuild.faq.when.li3_desc')}</li>
                <li><strong>{t('advanced.rebuild.faq.when.li4_title')}</strong> {t('advanced.rebuild.faq.when.li4_desc')}</li>
              </ul>
            </AccordionContent>
          </AccordionItem>

          <AccordionItem value="time">
            <AccordionTrigger>{t('advanced.rebuild.faq.time.title')}</AccordionTrigger>
            <AccordionContent className="space-y-2 text-sm">
              <p><strong>{t('advanced.rebuild.faq.time.depends')}</strong></p>
              <ul className="list-disc list-inside space-y-1 ml-2">
                <li>{t('advanced.rebuild.faq.time.li1')}</li>
                <li>{t('advanced.rebuild.faq.time.li2')}</li>
                <li>{t('advanced.rebuild.faq.time.li3')}</li>
                <li>{t('advanced.rebuild.faq.time.li4')}</li>
              </ul>
              <p className="mt-2"><strong>{t('advanced.rebuild.faq.time.rates')}</strong></p>
              <ul className="list-disc list-inside space-y-1 ml-2">
                <li><strong>{t('advanced.rebuild.faq.time.rates_local_title')}</strong> {t('advanced.rebuild.faq.time.rates_local_desc')}</li>
                <li><strong>{t('advanced.rebuild.faq.time.rates_cloud_title')}</strong> {t('advanced.rebuild.faq.time.rates_cloud_desc')}</li>
                <li><strong>{t('advanced.rebuild.faq.time.rates_sources_title')}</strong> {t('advanced.rebuild.faq.time.rates_sources_desc')}</li>
              </ul>
              <p className="mt-2"><em>{t('advanced.rebuild.faq.time.example')}</em></p>
            </AccordionContent>
          </AccordionItem>

          <AccordionItem value="safe">
            <AccordionTrigger>{t('advanced.rebuild.faq.safe.title')}</AccordionTrigger>
            <AccordionContent className="space-y-2 text-sm">
              <p><strong>{t('advanced.rebuild.faq.safe.lead_strong')}</strong> {t('advanced.rebuild.faq.safe.lead_tail')}</p>
              <ul className="list-disc list-inside space-y-1 ml-2">
                <li>✅ <strong>{t('advanced.rebuild.faq.safe.li1_title')}</strong> {t('advanced.rebuild.faq.safe.li1_desc')}</li>
                <li>✅ <strong>{t('advanced.rebuild.faq.safe.li2_title')}</strong> {t('advanced.rebuild.faq.safe.li2_desc')}</li>
                <li>✅ <strong>{t('advanced.rebuild.faq.safe.li3_title')}</strong> {t('advanced.rebuild.faq.safe.li3_desc')}</li>
                <li>✅ <strong>{t('advanced.rebuild.faq.safe.li4_title')}</strong> {t('advanced.rebuild.faq.safe.li4_desc')}</li>
              </ul>
              <p className="mt-2">⚠️ <strong>{t('advanced.rebuild.faq.safe.warn_title')}</strong> {t('advanced.rebuild.faq.safe.warn_desc')}</p>
            </AccordionContent>
          </AccordionItem>
        </Accordion>
      </CardContent>
