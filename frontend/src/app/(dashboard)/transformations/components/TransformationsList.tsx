...
export function TransformationsList({ transformations, isLoading, onPlayground }: TransformationsListProps) {
  ...
  if (!transformations || transformations.length === 0) {
    return (
      <EmptyState
        icon={Wand2}
        title={t('transformations.empty.title')}
        description={t('transformations.empty.desc')}
        action={
          <Button onClick={() => handleOpenEditor()}>
            <Plus className="h-4 w-4 mr-2" />
            {t('transformations.create_new')}
          </Button>
        }
      />
    )
    ...
    <>
      <div className="space-y-6">
        <div className="flex justify-between items-center">
          <h2 className="text-lg font-semibold">{t('transformations.yours')}</h2>
          <Button onClick={() => handleOpenEditor()}>
            <Plus className="h-4 w-4 mr-2" />
            {t('transformations.create_new')}
          </Button>
